...
Most of the web applications verify function level access rights before making that functionality accessible to the user. However, if the same access control checks are not performed on the server, hackers are able to penetrate into the application without proper authorization.</p><br>
...
Detecting such flaws is easy. The hardest part is identifying which pages (URLs) or functions exist to attack.</p><br>

<span id="c"><b>Are You Vulnerable To 'Missing Function Level Access Control'?</b></span><br>
<UL>
...
4. Using a proxy, browse your application with a privileged role. Then revisit restricted pages using a less privileged role. If the server responses are alike, you're probably vulnerable.</p><br>

<span id="c"><b>Example of a Attack Scenario:</b></span><br>
<ul>
...
<li>http://example.com/app/getappInfo (The page result a user gets)</li>
<li>http://example.com/app/admin_getappInfo  (The page result a Attacker gets after manupilating the URL)</li>
