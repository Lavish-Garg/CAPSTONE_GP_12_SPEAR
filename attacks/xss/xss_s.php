...
<center><h1>Stored XSS</h1></center>
<body>
  <br><br><br>
  <center>
</table>
...
<br><h4>This will occur when an attacker injects browser executable code within a single HTTP response. The injected attack will be stored within the server application itself.<h4>
</center>
 <!-- page end -->
</div>
...
if( isset( $_POST[ 'btnSign' ] ) ) {
  ...
  $message = ((isset($link) && is_object($link)) ? mysqli_real_escape_string($link,  $message ) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""));

  // Sanitize name input
  $name = ((isset($link) && is_object($link)) ? mysqli_real_escape_string($link,  $name ) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""));

  // Update database
  $query  = "INSERT INTO xss ( comment, name ) VALUES ( '$name' , '$message' );";
  $result = mysqli_query($link, $query ) or die( '<pre>' . ((is_object($link)) ? mysqli_error($link) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)) . '</pre>' );
